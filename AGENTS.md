...
13. **CUDA Graph capture (M=1) re-evaluated** — Original diagnosis (session 30): `attention_decode_gqa` wrapper calls `cudaFuncSetAttribute` (smem config) + H2D pinned `cudaMemcpyAsync` (seq_pos) conflict with `cudaStreamCaptureModeGlobal`. Illegal memory access during capture. **llama.cpp analysis (session 33): llama.cpp uses `cudaStreamCaptureModeRelaxed` (not `Global`). Their `CUDA_SET_SHARED_MEMORY_LIMIT` macro calls `cudaFuncSetAttribute` once before capture via static guard — this works in Relaxed mode. Mode difference may explain our failure.** Per-kernel fused path (181.4 t/s) remains production target for now, but M=1 CUDA Graph may be salvageable with `Relaxed` capture mode.
