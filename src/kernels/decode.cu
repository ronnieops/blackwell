...
// update_kv_kernel_pos: same as update_kv_kernel but seq_pos is a kernel argument
// (no device pointer dereference, no H2D copy race in tight loops)
__global__ void update_kv_kernel_pos(
    float* __restrict__ k_cache,
    float* __restrict__ v_cache,
    const float* __restrict__ k_new,
    const float* __restrict__ v_new,
...
// Same as attn_batched_kernel but seq_pos is a kernel argument (no H2D copy race)
__launch_bounds__(128, 2)
__global__ void attn_batched_kernel_pos(
    float* __restrict__ output,
    const float* __restrict__ Q,
    const float* __restrict__ K_cache_base,
    const float* __restrict__ V_cache_base,
...
// Direct seq_pos variant: no H2D copy, no pinned buffer race.
// Use in tight loops (e.g. prefill) where multiple positions are written sequentially.
cudaError_t update_kv_cache_pos(
    ...
    update_kv_kernel_pos<<<blocks, threads, 0, stream>>>(
        k_cache, v_cache,
        k_new, v_new,
