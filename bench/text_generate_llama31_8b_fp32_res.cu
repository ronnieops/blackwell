...
// Removes all quantize_int4 calls. Uses gemv_fp32_int4_warp instead of gemv_int4_warp.
//
// Llama 3.1 8B dimensions: NL=32, H=4096, I=14336, nqh=32, nkv=8, hd=128, V=128256
//
// Adapted from text_generate_llama31_8b.cu (Llama 3.1 8B INT4)
//
// Build:
//   cmake --build build --target text_generate_llama31_8b
...
int main(int argc, char** argv) {
    ...
    printf("# Text Generation — Llama 3.1 8B INT4 FP32 Residual\n");
    printf("  Weights: %s\n", wdir);
    printf("  Device: %s\n", P.name);
    ...
    for(int step=0;step<total;++step){
        ...
        for(int l=0;l<NL;++l){
            ...
            // QKV projections: FP32 acts × INT4 weights (no activation quant)
            cudaError_t _e1, _e2, _e3;
            _e1=blackwell::kernels::gemv_fp32_int4_warp(d_Q,(const float*)d_xi_f,W[l].q.d,W[l].q.sc,H,Q,st);
            cudaStreamSynchronize(st);
            _e2=blackwell::kernels::gemv_fp32_int4_warp(d_K,(const float*)d_xi_f,W[l].k.d,W[l].k.sc,H,KV,st);
            cudaStreamSynchronize(st);
            _e3=blackwell::kernels::gemv_fp32_int4_warp(d_V,(const float*)d_xi_f,W[l].v.d,W[l].v.sc,H,KV,st);
            ...
            // Wo projection: FP32 attn × INT4 weights
            ...
            // MLP gate + up: FP32 acts × INT4 weights
            ...
            // SwiGLU in FP32 (no quantize)
            blackwell::kernels::apply_swiglu(d_gate, d_gate, d_up, I, st);

            // Down projection: FP32 SwiGLU out × INT4 weights
        ...
        if(step>=gen_start-1){
            ...
            // lm_head: FP32 acts × INT4 weights (no activation quant)
            die(blackwell::kernels::gemv_fp32_int4_warp(d_logits,(const float*)d_xi_f,lm_head_w.d,lm_head_w.sc,H,V,st),"lm_head");
