// bench_ppl_int8_per_row.cu — Per-row INT8 weights + FP32 activations PPL benchmark
// Tests: does per-row scaling improve PPL vs block-16 (18.65)?
// Loads per-row INT8 weights → gemv_int8_per_row (no activation quantization)
...
static Int8W load_int8_per_row(const char* prefix) {
    ...
    // Load per-row scales (header: [K, N, 1, 1, N])
    snprintf(p, 512, "%s.scale_t", prefix);
    f = fopen(p, "rb"); if (!f) { fprintf(stderr, "FAIL open %s\n", p); exit(1); }
    int hs[5]; fread(hs, 4, 5, f);
    int nscales = hs[4];  // N scales (one per row)
    std::vector<float> scales(nscales); fread(scales.data(), 4, nscales, f); fclose(f);

    Int8W w{K, N, nullptr, nullptr};
    AL(cudaMalloc(&w.d, sz)); AL(cudaMemcpy(w.d, i8_data.data(), sz, cudaMemcpyHostToDevice));
    AL(cudaMalloc(&w.sc, nscales * 4)); AL(cudaMemcpy(w.sc, scales.data(), nscales * 4, cudaMemcpyHostToDevice));
    return w;
}
...
static void decode_step_per_row(float* d_residual, int seq_pos, int l,
    ...
    AL(blackwell::kernels::gemv_int8_per_row(d_Q, d_x_in, L.q.d, L.q.sc, H, Q, st));
    AL(blackwell::kernels::gemv_int8_per_row(d_K, d_x_in, L.k.d, L.k.sc, H, KV, st));
    AL(blackwell::kernels::gemv_int8_per_row(d_V, d_x_in, L.v.d, L.v.sc, H, KV, st));
    ...
    AL(blackwell::kernels::gemv_int8_per_row(d_proj, d_attn, L.o.d, L.o.sc, Q, H, st));
    ...
    AL(blackwell::kernels::gemv_int8_per_row(d_gate, d_x_in, L.gate.d, L.gate.sc, H, ID, st));
    AL(blackwell::kernels::gemv_int8_per_row(d_up, d_x_in, L.up.d, L.up.sc, H, ID, st));
    AL(blackwell::kernels::apply_swiglu(d_mlp, d_gate, d_up, ID, st));
    AL(blackwell::kernels::gemv_int8_per_row(d_proj, d_mlp, L.down.d, L.down.sc, ID, H, st));
    AL(blackwell::kernels::vector_add_fp32(d_proj, d_proj, d_residual, H, st));
    AL(cudaMemcpyAsync(d_residual, d_proj, H * 4, cudaMemcpyDeviceToDevice, st));
}
...
int main(int argc, char** argv) {
    ...
    const char* wdir = "weights_int8_per_row";
    int NL=28, H=2048, Q=2048, KV=1024, ID=6144, V=151936, nqh=16, nkv=8, hd=128;
    printf("Per-row INT8 weight-only + FP32 activation PPL Benchmark\n");
    ...
    // Load embed on host (per-row scales: 1 per output row)
    std::vector<int8_t> h_emb_i8((size_t)V * H);
    std::vector<float> h_emb_sc(V);
    {
        ...
        fread(hh,4,5,f); size_t ns=(size_t)hh[4]; h_emb_sc.resize(ns);
        fread(h_emb_sc.data(),4,ns,f); fclose(f);
    }
    // Pre-compute FP32 embed for fast host lookup (per-row dequant)
    ...
    for (int l=0;l<NL;l++) {
        ...
        snprintf(p,256,"%s/%d_self_attn.q_proj",wdir,l); L.q=load_int8_per_row(p);
        snprintf(p,256,"%s/%d_self_attn.k_proj",wdir,l); L.k=load_int8_per_row(p);
        snprintf(p,256,"%s/%d_self_attn.v_proj",wdir,l); L.v=load_int8_per_row(p);
        snprintf(p,256,"%s/%d_self_attn.o_proj",wdir,l); L.o=load_int8_per_row(p);
        snprintf(p,256,"%s/%d_mlp.gate_proj",wdir,l); L.gate=load_int8_per_row(p);
        snprintf(p,256,"%s/%d_mlp.up_proj",wdir,l);   L.up=load_int8_per_row(p);
        snprintf(p,256,"%s/%d_mlp.down_proj",wdir,l); L.down=load_int8_per_row(p);
    ...
    for(int s=0;s+1<N;s++){
        ...
        for(int l=0;l<NL;l++) decode_step_per_row(d_res,s,l,H,Q,KV,ID,V,nqh,nkv,hd,d_xin,d_Q,d_K,d_V,d_attn,d_proj,d_gate,d_up,d_mlp,layers[l],d_kc,d_vc,st);
        AL(blackwell::kernels::fused_rmsnorm(d_xin,d_res,d_fn,H,1e-6f,st));
        gemv_fp32(d_logits,d_xin,emb.d,H,V,st); AL(cudaGetLastError());
        // Debug: print first logits on first token
        if (s == 0) {
            float dbg[10]; cudaMemcpy(dbg, d_logits, 40, cudaMemcpyDeviceToHost);
            fprintf(stderr, "First token logits: ");
            for (int i = 0; i < 10; i++) fprintf(stderr, "%f ", dbg[i]);
            float mx = dbg[0];
            for (int i = 0; i < 10; i++) if (dbg[i] > mx) mx = dbg[i];
            fprintf(stderr, " max(first10)=%f\n", mx);
        }
