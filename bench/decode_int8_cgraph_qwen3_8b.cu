...
int main(int argc, char** argv) {
    ...
    printf("# INT8 CUDA Graph Decode Benchmark — Qwen3-8B\n");
    ...
    for (int l = 0; l < num_layers; ++l) {
        char p[256];
        snprintf(p,256,"weights_int8_qwen3_8b/%d_self_attn.q_proj",l); lw[l].q = upload(p);
        snprintf(p,256,"weights_int8_qwen3_8b/%d_self_attn.k_proj",l); lw[l].k = upload(p);
        snprintf(p,256,"weights_int8_qwen3_8b/%d_self_attn.v_proj",l); lw[l].v = upload(p);
        snprintf(p,256,"weights_int8_qwen3_8b/%d_self_attn.o_proj",l); lw[l].o = upload(p);
        snprintf(p,256,"weights_int8_qwen3_8b/%d_mlp.gate_proj",l);  lw[l].g = upload(p);
        snprintf(p,256,"weights_int8_qwen3_8b/%d_mlp.up_proj",l);    lw[l].u = upload(p);
        snprintf(p,256,"weights_int8_qwen3_8b/%d_mlp.down_proj",l);  lw[l].d = upload(p);
    }
