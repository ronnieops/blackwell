// bench/decode_int4_batched_attn_qwen3_8b.cu — INT4 batched decode, Qwen3-8B
//
// INT4 M-sequence decode with batched GEMV + batched attention for Qwen3-8B.
// 36 layers, H=4096, Q=4096, KV=1024, I=12288, nqh=32, nkv=8, hd=128.
//
// Build:
//   /usr/local/cuda-13.3/bin/nvcc -O3 -std=c++17 \
//     -gencode=arch=compute_120a,code=sm_120a \
//     -I include bench/decode_int4_batched_attn_qwen3_8b.cu build/libblackwell_kernels.a \
//     -o bench/decode_int4_batched_attn_qwen3_8b
...
int main(int argc, char** argv) {
    ...
    printf("# INT4 Batched Attn Decode — Qwen3-8B, M=%d\n", M);
    ...
    for (int l = 0; l < num_layers; ++l) {
        char p[256];
        snprintf(p,256,"weights_int4_qwen3_8b/%d_self_attn.q_proj",l); lw[l].q=upload_w4(p);
        snprintf(p,256,"weights_int4_qwen3_8b/%d_self_attn.k_proj",l); lw[l].k=upload_w4(p);
        snprintf(p,256,"weights_int4_qwen3_8b/%d_self_attn.v_proj",l); lw[l].v=upload_w4(p);
        snprintf(p,256,"weights_int4_qwen3_8b/%d_self_attn.o_proj",l); lw[l].o=upload_w4(p);
        snprintf(p,256,"weights_int4_qwen3_8b/%d_mlp.gate_proj",l);  lw[l].g=upload_w4(p);
        snprintf(p,256,"weights_int4_qwen3_8b/%d_mlp.up_proj",l);    lw[l].u=upload_w4(p);
        snprintf(p,256,"weights_int4_qwen3_8b/%d_mlp.down_proj",l);   lw[l].d=upload_w4(p);
    }
    ...
    cudaMalloc(&d_mlp_sc_b, (size_t)(H/16) * 4);
    cudaMalloc(&d_rn, H * 4);
    ...
    printf("\n=== Results ===\n");
    printf("Total:  %.3f ms (%d iters, %d layers, M=%d)\n", ms_total, iters, num_layers, M);
    printf("Layer:  %.3f ms, Seq: %.3f ms\n", ms_per_layer, ms_per_seq);
    printf("Per-seq t/s: %.1f (M=%d)\n", t_s, M);
    printf("Total t/s:  %.1f\n", t_s_total);
    printf("INT8 8B ref:   44.6 t/s\n");
    printf("Q4_K_M 8B ref: 82.56 t/s\n");
    printf("vs INT8 8B:    %.0f%%\n", t_s / 44.6f * 100.0f);
    printf("vs Q4_K_M 8B:  %.0f%%\n", t_s / 82.56f * 100.0f);
